...
inmates_df1 <-
    ...
    mutate(nm = sapply(seq_along(nm), function(t) null_weeder(nm[[t]]))) %>%
    mutate(nm = unlist(nm)) %>%
    mutate(street = sapply(seq_along(street), function(t) null_weeder(street[[t]]))) %>%
    ...
    mutate(race = sapply(seq_along(race), function(t) null_weeder(race[[t]]))) %>%
...
offenses <-
    ...
    select(rowid, x = ar) %>%
    unnest_longer(x) %>%
    pivot_wider(names_from = "x_id", values_from = "x", values_fn = list) %>%
    select(rowid, of) %>%
    unnest(of) %>%
    unnest_wider(of) %>%
    select(rowid, ol) %>%
